...
locals {
  ...
  consistency_group_boot_snapshot_crn = one(local.consistency_group_boot_snapshots[*].crn)
  ...
  consistency_group_snapshot_to_volume_map = {
    ...
    volume.name => one([for snap in data.ibm_is_snapshot.snapshots_from_group : snap.crn if contains(snap.service_tags, format("%s%s", "is.instance:attachment_index_", tostring(idx + 1)))])
  }
}
