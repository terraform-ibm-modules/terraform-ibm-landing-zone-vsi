...
variable "existing_kms_instance_crn" {
  ...
  validation {
    condition     = var.existing_kms_instance_crn != null || var.existing_boot_volume_kms_key_crn != null
    error_message = "Either 'existing_kms_instance_crn' or 'existing_boot_volume_kms_key_crn' should be passed."
  }
}
...
variable "force_delete_kms_key" {
  ...
  default     = false
  nullable    = false
  description = "If creating a new KMS key, toggle whether it should be force deleted on undeploy even if it is in use."
}

variable "skip_block_storage_kms_iam_auth_policy" {
  type        = bool
  description = "Set to true to skip the creation of an IAM authorization policy that permits all Storage Blocks to read the encryption key from the KMS instance."
  default     = false
}
