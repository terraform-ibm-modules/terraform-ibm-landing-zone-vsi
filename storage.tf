    ...
    for subnet in range(length(var.subnets)) : [
      ...
      for count in range(var.vsi_per_subnet) : [
        ...
        for idx, volume in(var.block_storage_volumes) :
        {
          ...
          snapshot_crn = try(coalesce(volume.snapshot_crn, lookup(local.consistency_group_snapshot_to_volume_map, volume.name, null)), null)
          tags         = volume.tags
        }
      ]
    ]
...
resource "ibm_is_volume" "volume" {
  ...
  tags                = var.resource_tags != null ? distinct(concat(var.resource_tags, each.value.tags)) : null
  access_tags         = var.access_tags
  source_snapshot_crn = each.value.snapshot_crn
}
