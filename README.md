...
| <a name="input_use_boot_volume_key_as_default"></a> [use\_boot\_volume\_key\_as\_default](#input\_use\_boot\_volume\_key\_as\_default) | Set to true to use the boot\_volume\_encryption\_key as default for all volumes, overriding any encryption\_key specified. If set to false, the encryption\_key provided in `block_storage_volumes.encryption_key` will be used instead. | `bool` | `false` | no |
