...
1. Specify individual Snapshot Ids using the `boot_volume_snapshot_crn` and `block_storage_volumes.snapshot_crn` input variables
...
| Name | Source | Version |
|------|--------|---------|
| <a name="module_existing_boot_volume_kms_key_crn_parser"></a> [existing\_boot\_volume\_kms\_key\_crn\_parser](#module\_existing\_boot\_volume\_kms\_key\_crn\_parser) | terraform-ibm-modules/common-utilities/ibm//modules/crn-parser | 1.4.1 |
...
| <a name="output_consistency_group_storage_snapshot_crns"></a> [consistency\_group\_storage\_snapshot\_crns](#output\_consistency\_group\_storage\_snapshot\_crns) | Map of attached storage volumes requested, and the Snapshot crn that will be used, determined from an optionally supplied consistency group, and mapped |
